...
@Component({
  selector: 'app-catastrophic-event',
  templateUrl: './catastrophic-event.component.html',
  styleUrls: ['./catastrophic-event.component.css']
})
export class CatastrophicEventComponent implements OnInit {
  CatastrophicEvent =
    {
      name: '',
      place: '',
  ...
  GetAllGroupOfCompanies() {
    this.apiService.accidentAndEmergency.GetAllCatastrophic().subscribe((res: any) => {
      this.CatastrophicEventColl = res.data.sort(this.ArrayInDecending('id'));
  ...
  Save() {
    ...
    if (this.CatastrophicEvent.id > 0) {
      this.apiService.accidentAndEmergency.updateCatastrophic(this.CatastrophicEvent).subscribe((res: any) => {
        this.CatastrophicEvent.id = -1;
        this.GetAllGroupOfCompanies();
        this.myForm.resetForm();
      })
    } else {
      this.apiService.accidentAndEmergency.SaveCatastrophic(this.CatastrophicEvent).subscribe((res: any) => {
        this.GetAllGroupOfCompanies();
        this.myForm.resetForm();
      })
    }
  ...
  EditRecord(id, $element: any) {
    let a = this.CatastrophicEventColl.find(x => x.id === id);
    this.CatastrophicEvent.name = a.name;
    this.CatastrophicEvent.id = a.id;
    this.CatastrophicEvent.place = a.place;
    this.CatastrophicEvent.description = a.description;
    this.CatastrophicEvent.eventDate = a.eventDate.split('T')[0];
    this.CatastrophicEvent.status = a.status;
  ...
  DeleteData(event) {
    ...
    this.apiService.accidentAndEmergency.DeleteCatastrophic(this.DeletedIndex).subscribe((res: any) => {
      this.GetAllGroupOfCompanies();
    });
  }
