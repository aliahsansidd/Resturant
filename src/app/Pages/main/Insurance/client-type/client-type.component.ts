...
export class ClientTypeComponent implements OnInit {
  ...
  GetAllClientGroup() {
    this.apiService.insuranceService.GetAllClientGroup().subscribe((res: any) => {
      this.ClientGroupCollection = res.data.sort(this.ArrayInDecending('id'));
    });
  }
  ...
  Save() {
    ...
    if (this.ClientType.id > 0) {
      this.apiService.insuranceService.updateClientType(this.ClientType).subscribe((res: any) => {
        this.ClientType.id = -1;
        this.GetAllClientType();
        this.myForm.resetForm();
      })
    } else {
      this.apiService.insuranceService.SaveClientType(this.ClientType).subscribe((res: any) => {
        this.GetAllClientType();
        this.myForm.resetForm();
      })
    }
  ...
  EditRecord(id, $element: any) {
    let a = this.ClientTypeColl.find(x => x.id === id);
    this.ClientType.name = a.name;
    this.ClientType.remarks = a.remarks;
    this.ClientType.id = a.id;
    this.ClientType.clientGroupID = a.clientGroupID;
    this.ClientType.status = a.status;
  ...
  DeleteData(event) {
    ...
    this.apiService.insuranceService.DeleteClientType(this.DeletedIndex).subscribe((res: any) => {
      this.GetAllClientType();
    });
  }
  GetAllClientType(): any {
    this.apiService.insuranceService.GetAllClientType().subscribe((res: any) => {
      this.ClientTypeColl = res.data;
