...
export class UnitOfMeasurementComponent implements OnInit {
  ...
  ProductMeasurementUnitCollection: any;
  constructor(private apiService: ApiService, private chRef: ChangeDetectorRef) { }

  ngOnInit() {
    this.GetAllProductMeasurementUnit();
  }

  GetAllProductMeasurementUnit() {
    this.apiService.pharmacyService.GetAllProductMeasurementUnit().subscribe((res: any) => {
      this.ProductMeasurementUnitCollection = res.data;
  ...
  Save() {
    ...
    if (this.MeasurementUnit.id > 0) {
      this.apiService.pharmacyService.updateProductMeasurementUnit(this.MeasurementUnit).subscribe((res: any) => {
        this.MeasurementUnit.id = -1;
        this.GetAllProductMeasurementUnit();
        this.myForm.resetForm();
      })
    } else {
      this.apiService.pharmacyService.SaveProductMeasurementUnit(this.MeasurementUnit).subscribe((res: any) => {
        this.GetAllProductMeasurementUnit();
        this.myForm.resetForm();
      })
    }
  ...
  EditRecord(id, $element: any) {
    let a = this.ProductMeasurementUnitCollection.find(x => x.id === id);
    this.MeasurementUnit.name = a.name;
    this.MeasurementUnit.id = a.id;
  ...
  DeleteData(event) {
    ...
    this.apiService.pharmacyService.DeleteProductMeasurementUnit(this.DeletedIndex).subscribe((res: any) => {
      this.GetAllProductMeasurementUnit();
    });
  }
