...
export class BrandNameComponent implements OnInit {
  ...
  Save() {
    ...
    if (this.Brand.id > 0) {
      this.apiService.pharmacyService.updateBrandName(this.Brand).subscribe((res: any) => {
        this.Brand.id = -1;
        this.GetAllBrandName();
        this.myForm.resetForm();
      })
    } else {
      this.apiService.pharmacyService.SaveBrandName(this.Brand).subscribe((res: any) => {
        this.GetAllBrandName();
        this.myForm.resetForm();
      })
    }
  ...
  EditRecord(id, $element: any) {
    let a = this.BrandNameCollection.find(x => x.id === id);
    this.Brand.name = a.name;
    this.Brand.id = a.id;
  ...
  DeleteData(event) {
    ...
    this.apiService.pharmacyService.DeleteBrandName(this.DeletedIndex).subscribe((res: any) => {
      this.GetAllBrandName();
    });
  }
