...
export class LabServicesComponent implements OnInit {
  ...
  save(service) {
    console.log(service);
    this.apiService.labService.createLabService(service).subscribe(res => {
      console.log(res);
    })
  }
  GetLabType() {
    this.labDepartments = this.apiService.labService.getAllLabDepartments().subscribe((res: any) => {
      console.log(res);
      this.labDepartments = res.data;
