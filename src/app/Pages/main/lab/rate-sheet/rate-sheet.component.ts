...
@Component({
  ...
  styleUrls: ['./rate-sheet.component.css']
})
export class RateSheetComponent implements OnInit {
  ...
  save() {
    ...
    this.apiService.labService.createRateSheet(this.rate).subscribe(res => {
      console.log(res);
      this.getRateSheet();
    })
  }
  ...
  getRateSheet() {
    this.apiService.labService.getAllRates().subscribe((res: any) => {
